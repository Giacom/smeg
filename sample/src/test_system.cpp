...
void TestSystem::Process(smeg::Entity &entity) {
	...
	transform.position.x += (f32)time.delta * 100.0f;

	transform.position.x = (fmod(transform.position.x * 2, 1024) - transform.position.x); 
	transform.position.y = (sin(transform.position.x * 0.01f + (f32)(time.current) * 0.001f) * 50);
	transform.position.z = transform.position.x;

	f32 size = (4.0f + sin((f32)time.current * 0.001f)) * 16.0f;
